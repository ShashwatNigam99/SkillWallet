...
LOGGER = logging.getLogger('userwallet.userwallet_client.peer_verification')
...
class PeerVerificationClass(object):
    ...
    # TODO pass caller object instead?
    # TODO separate transaction util?
        ...
        try:
            ...
            if request_list is not None:
                ...
                for txn_id in request_events:
                    ...
                    # resp = input("Process attestation request for address {} ? Y/N: ".format(peer_address))
    ...
    # Modified Mar 9: Definition changed -
    # old self.verify_peer_data(request_txn_id, peer_address)

    def verify_peer_data(self, request_txn_id=None):
        LOGGER.debug("inside verify_peer_data")
        # serve the requests coming from the event client and stored in the event database
        # get the requesting transaction id from the request to fetch the transaction data from the chain
        ...
        #         LOGGER.debug("Request transaction originating from address {} instead of {}".format(txn_signer_address,
        #                                                                                             peer_address))
        ...
        # Modified Mar 9: Verify digital_id.id_owner_public_key against peer_address as well
        # Commenting out the check as it seems redundant - this check is being done by digitalid_tp
        # if peer_address != hashing.get_pub_key_hash(digital_id.id_owner_public_key):
        #     print("Invalid Request: The transaction contains digital ID not owned by {}".format(peer_address))
        #     return False
        ...
        for attribute_field in attribute_fields:
            if attribute_field[0].name != 'others':
                ...
                # tests attribute_set_msg[field_name]
                # if field_name == 'name':
                #     LOGGER.debug("name : {}".format(digital_id.attribute_set.name))
                ...
                for field_name in attr_map:
                    ...
                    LOGGER.debug("{} : {}".format(field_name, attr_map[field_name]))
    ...
    def _send_peer_verification_response(self, peer_address, peer_txn_id, peer_id, operation_type):
        LOGGER.debug("Inside _send_peer_verification_response")
        transaction_list = []
        ...
        client_info.family_name = FAMILY_NAME_DIGITALID
        peer_verification_response.peer_info.CopyFrom(client_info)

        # TODO reward feature is to be added later
        # state_info_dict is updated only after transaction commitment
        # if self.state_info_dict['trust_score'] < digital_id_constants.MAX_CLIENT_TRUST_SCORE:
        #     peer_verification_response.verification_reward = digital_id_constants.PEER_VERIFICATION_REWARD
        #     # TODO FOLLOWING to be updated in state
        #     self.state_info_dict['trust_score'] = self.state_info_dict['trust_score'] + \
        #                                           digital_id_constants.PEER_VERIFICATION_REWARD
