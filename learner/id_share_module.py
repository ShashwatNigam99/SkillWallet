...
class DigitalIdSharingClass(object):
    ...
    def __init__(self, base_url, signer, public_key, to_address):
        ...
        self_address is the sender's block-chain public address
    ...
    def send_id_request(self, data_mode, contract_msg=None):
        ...
        # recv_id_from_address discloses who is seeking ID from whom. No untraceability
        ...
        output_address_list = [sharing_state_address]  # a negotiated address which anonymizes the receiver
    ...
    def send_id_response(self, data_mode=False):
        LOGGER.debug("Inside send_id_response()")
        LOGGER.debug("data_mode {}".format(data_mode))
        share_ID_transaction = ShareIDTransaction()
        ...
        # confirmation_txn = id_state_data['ack_number']
        ...
        if data_mode:
            pass  # prepare secret codes to share with the receiver off-chain

        # commented id_info population
        # id_info = Id_info()
        # id_info.id_creating_pub_key = saved_id.id_owner_public_key
        # id_info.id_confirmation_txn = confirmation_txn

        # commented digital_id_hash population
        ...
        input_address_list = [self_state_address, sharing_state_address]  # sharing_state_address is again the negotiated address
